...
class island
{
      ...
      static inline island* GetTopAdjacentIsland(const vector<int> *pTopVec, island *pTopIsland, pair<int, int> && pointxy)
      {
            if ((*pTopVec)[pointxy.first])
            {
                  if (pTopIsland)
                        return pTopIsland;
...
public:
      ...
      bool isNearByTop(pair<int, int> &pointxy) const
      {
            for (const auto &point_vec: m_point_vec_vec)
            {
                  if (binary_search(point_vec.begin(), point_vec.end(), pointxy))
                        return true;
            }
      ...
      static int GetNumberOfIslands(vector<vector < int>> &vmap)
      {
            ...
            i = 1;
            for_each(next(prevline->begin()), prevline->end(), [& ](auto & x)
            {
            	    //first line
            ...
            for_each(next(vmap.begin()), vmap.end(), [& ](auto & yvec)
            {
                  ...
                  for_each(yvec.begin(), yvec.end(), [& ](auto & value)
                  {
                        if (value)	// we are on the land..
                        {
                              pTopIsland = GetTopAdjacentIsland(prevline, pTopIsland, make_pair(i, j - 1));
